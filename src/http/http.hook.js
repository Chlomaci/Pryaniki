export const useHttp = () => {
    ...
    const onDataDelete = async ({url, personalToken}, method = 'POST') => {
        try {
            const response = await fetch(url, {method, headers: {'Content-Type': 'application/json', 'x-auth': personalToken}});
    ...
    const onDataEditing = async ({url, compSigDate, compSignName, docName, docStatus, docType, empNum, empSigDate, empSigName, personalToken}, method = 'POST') => {
    ...
    return {onAuth, onDataLoading, onDataAdding, onDataDelete, onDataEditing}
}